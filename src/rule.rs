...
pub mod leduc {
    ...
    pub fn rule() -> Rule {
        ...
        leduc.info_partitions.insert(Player::P1, InformationPartition::new());
        leduc.info_partitions.insert(Player::P2, InformationPartition::new());
        leduc.build();
                ...
                if let Some(val) = terminal_val(&s) {
                    ...
                    for (action, prob) in trans {
                        ...
                        if action.starts_with("Deal") {
                            next_state.card.insert(Player::P1, action.chars().nth(4).unwrap());
                            next_state.card.insert(Player::P2, action.chars().nth(5).unwrap());
                    ...
                    self.nodes.insert(s.node_id, Node::NonTerminal{ player: Player::C, edges });
                    ...
                    if self.observations.contains_key(&obs) {                        
                        ...
                        self.info_partitions.get_mut(&player).unwrap().get_mut(&info_set_id).unwrap().push(s.node_id);
                    } else {
                        let info_set_id = InformationSetId::new(self.info_set_id);
                        self.observations.insert(obs, self.info_set_id);
                        self.info_set_id += 1;
                        self.info_partitions.get_mut(&player).unwrap().insert(info_set_id, vec![s.node_id]);
                    }
                    ...
                    self.nodes.insert(s.node_id, Node::NonTerminal{ player, edges });
                }
            }
        }
    }
    ...
    fn transition(s: &State) -> Option<BTreeMap<&'static str, f64>> {
        ...
        if s.history[n-1] == "Call" || (n >= 2 && s.history[n-1] == "Check" && s.history[n-2] == "Check") {
    ...
    fn terminal_val(s: &State) -> Option<i32> {
        ...
        if s.history[n-1] == "Fold" {
            ...
            return Some(
                - myself.sign() * (s.bet[&myself] + (s.pot >> 1))
            );
        }
    ...
    fn possible_actions(s: &State) -> Option<(Player, Vec<&'static str>)> {
        ...
        match s.last_player.unwrap() {
            Player::P1 => Some((Player::P2, match s.history[n-1] {
            ...
            Player::P2 => Some((Player::P1, match s.history[n-1] {
                    "Raise" => {
